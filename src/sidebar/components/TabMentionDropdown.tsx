 ...
 */
export const TabMentionDropdown: React.FC<TabMentionDropdownProps> = memo(
  ({
    tabs,
    onSelect,
    position,
    ...
    // Pre-compute favicon URLs for all tabs to avoid re-computation on every render
    const tabFavicons = useMemo(() => {
      const favicons = new Map<number, string>();
      tabs.forEach(tab => {
        const result = getDomSafeFaviconUrlSync(tab.url, tab.favIconUrl, 16);
        favicons.set(tab.id, result.url);
      });
      return favicons;
    }, [tabs]);
    ...
    const renderTabOption = useCallback(
            ...
            <div className="tab-mention-dropdown__option-content">
              <div className="tab-mention-dropdown__header-main">
                <div className="tab-mention-dropdown__title-wrapper">
                  <img
                    src={
                      tabFavicons.get(tab.id) ||
                      getDomSafeFaviconUrlSync(tab.url, tab.favIconUrl, 16).url
                    }
                    alt=""
                    ...
                    decoding="async"
                    onError={e => {
                      // Fallback to Google service on error
                      const fallback = getDomSafeFaviconUrlSync(tab.url, undefined, 16);
                      if (fallback.url !== (e.target as HTMLImageElement).src) {
                        (e.target as HTMLImageElement).src = fallback.url;
                      } else {
                        // As last resort hide the icon
      ...
      [handleSelect, shouldVirtualize, tabFavicons]
    );
...
TabMentionDropdown.displayName = 'TabMentionDropdown';
