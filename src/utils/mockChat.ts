 ...
 */
export function generateMockResponse(type: MockResponseType, prompt?: string): MockResponse {
  ...
  if (prompt && content) {
    ...
    if (type === 'text') {
      if (lowerPrompt.includes('react')) {
        content =
          'React is a powerful JavaScript library for building user interfaces. It uses a component-based architecture that makes it easy to create reusable UI elements and manage complex application state.';
      } else if (lowerPrompt.includes('python')) {
        content =
          "Python is an excellent programming language known for its simplicity and readability. It's great for beginners but powerful enough for complex applications in data science, web development, and automation.";
      } else if (lowerPrompt.includes('javascript')) {
        content =
          "JavaScript is the language of the web! It's versatile, running both in browsers and on servers. Modern JavaScript includes many powerful features that make development more efficient and enjoyable.";
      }
    }
  }
...
const CONVERSATION_SCENARIOS = {
  ...
  coding: [
    { role: 'user' as MessageRole, content: 'Can you help me debug this JavaScript code?' },
    {
      role: 'assistant' as MessageRole,
      content:
        "I'd be happy to help you debug your JavaScript code! Please share the code you're having trouble with, and let me know what error messages or unexpected behavior you're experiencing.",
    },
    {
  ...
  error: [
    { role: 'user' as MessageRole, content: 'Can you analyze this large dataset for me?' },
    {
      ...
      error:
        'Failed to process the request. The dataset may be too large or in an unsupported format.',
    },
    { role: 'user' as MessageRole, content: 'Let me try with a smaller sample instead.' },
    {
      role: 'assistant' as MessageRole,
      content:
        "That's a good approach! Working with a smaller sample will be much more manageable. Please share the sample data and let me know what kind of analysis you're looking for.",
    },
  ],
...
export function generateMockConversation(
  ...
  const { messageCount, includeErrors = scenario === 'error' } = options;
