...
describe('EncryptionService', () => {
  ...
  describe('Data Encryption/Decryption', () => {
    ...
    it('should encrypt and decrypt object data', async () => {
      ...
      const { encryptObject, decryptObject, isValidEncryptedData } = await import(
        '@/security/crypto'
      );
  ...
  describe('Error Handling', () => {
    ...
    it('should handle chrome storage errors', async () => {
      ...
      await expect(service.initialize('secure-password')).rejects.toThrow(
  ...
  describe('Memory Cleanup', () => {
    ...
    it('should clear keys on explicit memory cleanup', async () => {
      ...
      vi.mocked(mockManager.deriveKey).mockResolvedValueOnce(key1).mockResolvedValueOnce(key2);
